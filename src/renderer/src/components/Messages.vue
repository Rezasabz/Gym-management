        ...
        <div class="card-body p-6 md:p-8 space-y-6 rtl">
          ...
          <div class="form-control">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
              <label class="card bg-gradient-to-br from-blue-50 to-purple-50 hover:from-blue-100 hover:to-purple-100 transition-all cursor-pointer border border-blue-100 rounded-xl">
            ...
            <div class="overflow-x-auto shadow-md rounded-xl border border-blue-100">
              <table class="table table-zebra w-full text-right">
                ...
                <tbody>
                  ...
                  <tr v-for="user in filteredUsers" :key="user.id" class="hover:bg-blue-50 transition-colors">
                    <td>
          ...
          <div v-if="resultMessage" class="alert mt-4 rounded-xl" :class="resultClass">
            <div class="flex items-center gap-2">
              <svg v-if="resultClass.includes('alert-success')" xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-green-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
...
export default {
  ...
  computed: {
    ...
    selectedCount() {
      ...
      if (this.sendTo === 'active') return this.users.filter(user => user.status === 'فعال').length;
  ...
  methods: {
    ...
    async sendSms() {
      ...
      if (this.message.trim() !== '') {
        ...
        await this.showSwal(
          'خطا', 
          'خطا در ارسال پیامک. لطفا دوباره تلاش کنید', 
          'error'
        );
      } finally {
